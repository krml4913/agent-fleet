...
| `fleet leader --respawn [--name LABEL] [--agent SPEC]` | Restart only the leader agent of the running session `fleet-<LABEL>`: a new leader window starts with the command line a fresh `fleet leader` would use (so current `leader_delivery` settings apply), the old leader window is closed, and the leader prompt is re-pasted. Driver windows and queued leader notifications are left alone; `scope` and `started_at` in `session.json` are kept. The agent defaults to the one the session was started with. Safe to run inside the leader pane itself (after quitting the agent, or by asking the leader to run it): it then finishes in the background, logging to `global/sessions/<LABEL>/leader-respawn.log`. |
