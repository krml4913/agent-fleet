...
Rules:
  ...
  - Leader relay (claude drivers under a claude leader): if `fleet-agent done` or `fleet-agent ask` prints a `[fleet] leader relay` block, send the text between its `BEGIN LEADER MESSAGE` / `END LEADER MESSAGE` markers to the session it names with your `SendMessage` tool right away — verbatim, not rephrased. fleet does not type that notification into the leader's pane, so this send is how the leader learns of it. If SendMessage fails, say so in outbox.md.
